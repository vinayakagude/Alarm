...
def audio_player_autoplay(audio_bytes, mime='audio/wav', key='aplayer', repeat_seconds=1):
    """Play audio and repeat until repeat_seconds elapse."""
    b64 = base64.b64encode(audio_bytes).decode()
    dur_ms = 1000 * max(1, int(repeat_seconds))
    html = f"""
    <audio id='{key}' preload='auto'></audio>
    <script>
      (function() {{
        var a=document.getElementById('{key}');
        a.src='data:{mime};base64,{b64}';
        var endTime=Date.now()+{dur_ms};
        function strike(){{ try{{ a.pause(); a.currentTime=0; a.play(); }}catch(e){{}} }}
        function loop(){{
          if(Date.now()<endTime){{ strike(); setTimeout(loop,2000); }}
...
st.caption('US/Eastern time. Set start and end, repeat every minute, choose sound, and duration.')

# Try to enable audio by default (muted autoplay "primes" the session)
    ...
    if sdt <= now <= edt:
        ...
        if elapsed % t['interval_min'] == 0:
            ...
            if hm not in t['fired']:
                ...
                t['fired'].append(hm)
                # Prevent refresh while the alarm is sounding so repeats aren't interrupted
                st.session_state.no_refresh_until = max(
                    st.session_state.no_refresh_until,
                    (now + dt.timedelta(seconds=t['play_seconds'])).timestamp(),
                )

# Auto-refresh: every second unless an alarm is currently playing
